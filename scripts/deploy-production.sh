...
helm_overrides=(
  ...
  --set-string "backup.postgres.caSecretName=${postgres_ca_secret}"
  ...
  --set-string "retention.postgres.caSecretName=${postgres_ca_secret}"
...
FLOWMESH_PROMETHEUS_RELEASE="${prometheus_release}" \
FLOWMESH_ALERTMANAGER_CONFIG_SECRET_NAME="${alertmanager_config_secret}" \
