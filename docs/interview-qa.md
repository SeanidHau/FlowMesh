...
Q: Outbox 发布器如何降低批量发送时的租约失效风险？
A: 认领批次在短事务中完成，发送前会以 `id + claimToken` 条件续租当前事件；如果续租影响行数为 0，说明事件已被其他实例接管，当前实例直接跳过发送。发布器仍保留发送超时、批量窗口和租约的启动校验，消费者 Inbox 幂等作为最后一道重复投递防线。
