...
- 外部通知投递已落地：站内通知与投递队列同事务提交，Webhook 使用 HTTPS、HMAC 签名、幂等键、租约认领、发送前续租、指数退避和死信；队列通过专用 `flowmesh_audit_delivery` `BYPASSRLS` 角色的安全函数跨租户调度，业务账号不直接读取投递队列。Webhook 成功但本地确认失败会单独计数并触发告警，便于发现潜在重复投递。
