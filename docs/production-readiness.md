...
- Gateway 已使用 Redis RateLimiter 对所有业务路由执行分布式令牌桶限流；生产入口必须覆写配置的客户端地址请求头，避免公网请求伪造限流身份。
...
- 提供只读 Kubernetes 生产 smoke test，验证六个 Deployment、提交 SHA 镜像、PDB/HPA/NetworkPolicy、运行时 Secret 和备份 CronJob；目标环境仍需实际执行并留存输出。
