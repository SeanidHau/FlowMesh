...
- 提供只读生产证据包校验：要求目标环境归档 Kubernetes smoke、外部依赖 HA、运行时观测、应用恢复、备份恢复、压测、跨租户安全回归和告警路由报告；校验器还要求每类报告包含对应检查命令、证据摘要和关键结果，并通过清单与 SHA-256 校验和防止缺项或篡改。生产验收默认强制执行该门禁，非生产预检必须显式设置 `FLOWMESH_REQUIRE_PRODUCTION_EVIDENCE=false` 才能跳过。
